...
__powerline() {
    ...
    ps1() {
        ...
        if [ $? -eq 0 ]; then
            local symbol="$COLOR_SUCCESS$SYMBOL_PROMPT_CHAR $COLOR_RESET"
        else
            local symbol="$COLOR_FAILURE$SYMBOL_PROMPT_CHAR $COLOR_RESET"
        fi

        local cwd="$COLOR_CWD\w$COLOR_RESET"
        local user_host="$COLOR_USER_HOST\u@\h$COLOR_RESET"
