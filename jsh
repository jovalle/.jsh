...
# Show git repository status with update check
# Usage: _git_repo_status <repo_dir>
# Shows: ✓ up-to-date, ✓ updated, or ! dirty status
_git_repo_status() {
  local repo_dir="$1"
  local before_hash after_hash
  local staged unstaged untracked dirty_summary

  # Capture commit hash before pull
  before_hash=$(git -C "${repo_dir}" rev-parse --short HEAD 2>/dev/null)

  # Attempt to pull
  if ! git -C "${repo_dir}" pull --rebase --quiet 2>/dev/null; then
    prefix_warn "Failed to pull updates"
    return 1
  fi

  # Capture commit hash after pull
  after_hash=$(git -C "${repo_dir}" rev-parse --short HEAD 2>/dev/null)

  # Check for dirty state (staged, unstaged, untracked)
  staged=$(git -C "${repo_dir}" diff --cached --numstat 2>/dev/null | wc -l | tr -d ' ')
  unstaged=$(git -C "${repo_dir}" diff --numstat 2>/dev/null | wc -l | tr -d ' ')
  untracked=$(git -C "${repo_dir}" ls-files --others --exclude-standard 2>/dev/null | wc -l | tr -d ' ')

  # Build dirty summary if any
  if [[ "${staged}" -gt 0 || "${unstaged}" -gt 0 || "${untracked}" -gt 0 ]]; then
    ...
    if [[ "${before_hash}" == "${after_hash}" ]]; then
      echo "${YLW}!${RST} Repository has uncommitted changes ${DIM}(${dirty_summary})${RST}"
    else
      echo "${YLW}!${RST} Updated ${DIM}${before_hash}${RST} → ${DIM}${after_hash}${RST} with uncommitted changes ${DIM}(${dirty_summary})${RST}"
    fi
  elif [[ "${before_hash}" == "${after_hash}" ]]; then
    prefix_success "Repository already up-to-date ${DIM}(${after_hash})${RST}"
  else
    prefix_success "Updated ${DIM}${before_hash}${RST} → ${DIM}${after_hash}${RST}"
  fi
  echo ""
}
...
# Sets global variables: BOOTSTRAP_SHELL_TOOLS, BOOTSTRAP_NVIM
...
_bootstrap_gather_selections() {
  ...
  # Check if jq/fzf/nvim already available
  local has_jq=false has_fzf=false has_nvim=false
  has jq && has_jq=true
  has fzf && has_fzf=true
  has nvim && has_nvim=true

  # Skip download section entirely if tools are installed or downloads impossible
  if [[ "${has_jq}" == true ]] && [[ "${has_fzf}" == true ]] && [[ "${has_nvim}" == true ]]; then
    echo ""
    echo "${CYN}Shell Tools:${RST}"
    echo "  ${GRN}✓${RST} jq, fzf, and nvim already installed"
    return 0
  fi
  ...
  if [[ "${can_download}" == false ]]; then
    ...
    echo "  ${DIM}  macOS: brew install jq fzf neovim${RST}"
    echo "  ${DIM}  Linux: apt install jq fzf neovim curl${RST}"
    return 0
  fi
  ...
  # --- Shell Tools (jq, fzf) ---
  if [[ "${has_jq}" == false ]] || [[ "${has_fzf}" == false ]]; then
  ...
  # --- Neovim ---
  if [[ "${has_nvim}" == false ]]; then
    echo ""
    echo "${CYN}Neovim + LazyVim:${RST}"
    echo ""
    echo "  ${BOLD}nvim${RST} - Hyperextensible Vim-based text editor"
    echo "  ${DIM}Includes LazyVim configuration (via submodule)${RST}"
    echo ""
    echo "  ${DIM}Pre-built binary from GitHub releases.${RST}"
    echo "  ${DIM}Alternative: brew install neovim (macOS) or dnf install neovim (Linux)${RST}"
    echo ""

    if _bootstrap_confirm "  Download Neovim?" "y"; then
      BOOTSTRAP_NVIM=true
    fi
  else
    echo ""
    echo "${CYN}Neovim:${RST}"
    echo "  ${GRN}✓${RST} nvim already installed"
  fi

  # --- Multi-platform binaries (for jssh portability) ---
  if [[ "${BOOTSTRAP_SHELL_TOOLS}" == true ]] || [[ "${BOOTSTRAP_NVIM}" == true ]]; then
    echo ""
...
_bootstrap_show_download_summary() {
  ...
  # Show nvim download
  if [[ "${BOOTSTRAP_NVIM:-false}" == true ]]; then
    has_downloads=true
    if [[ "${BOOTSTRAP_ALL_PLATFORMS:-false}" == true ]]; then
      echo "${CYN}Neovim (all platforms):${RST}"
      echo "  ${YLW}↓${RST} nvim - Neovim editor + LazyVim config"
      echo "  ${DIM}    Platforms: darwin-arm64, darwin-amd64, linux-arm64, linux-amd64${RST}"
    else
      local platform
      platform=$(_bootstrap_detect_platform)
      echo "${CYN}Neovim (from GitHub → lib/nvim/${platform}/):${RST}"
      echo "  ${YLW}↓${RST} nvim - Neovim editor + LazyVim config"
    fi
    echo ""
  fi
...
# Execute shell tools downloads (jq, fzf, nvim)
_bootstrap_download_shell_tools() {
  local has_downloads=false
  [[ "${BOOTSTRAP_SHELL_TOOLS:-false}" == true ]] && has_downloads=true
  [[ "${BOOTSTRAP_NVIM:-false}" == true ]] && has_downloads=true
  ...
  if [[ -f "${JSH_DIR}/src/deps.sh" ]]; then
    ...
    prefix_info "Install manually: brew install jq fzf neovim (macOS) or apt install jq fzf neovim (Linux)"
    return 1
  fi
  ...
  if [[ "${BOOTSTRAP_ALL_PLATFORMS:-false}" == true ]]; then
    ...
    # Also download nvim for all platforms if requested
    if [[ "${BOOTSTRAP_NVIM:-false}" == true ]]; then
      local platforms=("darwin-arm64" "darwin-amd64" "linux-arm64" "linux-amd64")
      local nvim_success=false
      for platform in "${platforms[@]}"; do
        if download_binary_for_platform "nvim" "${platform}"; then
          nvim_success=true
        else
          ((errors++))
        fi
      done
      # Setup lazyvim if at least one nvim download succeeded
      [[ "${nvim_success}" == true ]] && _setup_lazyvim
    fi
  else
    ...
    # Download nvim + setup lazyvim
    if [[ "${BOOTSTRAP_NVIM:-false}" == true ]]; then
      if ! command -v nvim >/dev/null 2>&1; then
        if download_binary "nvim"; then
          _setup_lazyvim
        else
          ((errors++))
        fi
      else
        prefix_success "nvim (system)"
        # Still setup lazyvim if nvim exists but lazyvim isn't configured
        _setup_lazyvim
      fi
    fi
  fi
  ...
  if [[ ${errors} -gt 0 ]]; then
    ...
    echo "  ${DIM}Install via package manager: brew install jq fzf neovim${RST}"
...
cmd_setup() {
  ...
  if [[ "${skip_git}" == true ]]; then
    ...
    if git -C "${JSH_DIR}" submodule update --init --depth 1 -- "${core_submodules[@]}" 2>/dev/null; then
...
# =============================================================================
# Feature Selection (Linux Full Experience)
# =============================================================================

# =============================================================================
# LazyVim Setup Helper
# =============================================================================

# Initialize lazyvim submodule and create config symlink
# Called only when nvim is being installed
_setup_lazyvim() {
  local lazyvim_dir="${JSH_DIR}/lib/lazyvim"
  local nvim_config="${XDG_CONFIG_HOME:-${HOME}/.config}/nvim"

  # Init lazyvim submodule if not already
  if [[ ! -f "${lazyvim_dir}/init.lua" ]]; then
    prefix_info "Initializing lazyvim submodule..."
    if ! git -C "${JSH_DIR}" submodule update --init --depth 1 -- lib/lazyvim 2>/dev/null; then
      prefix_warn "Failed to init lazyvim submodule"
      return 1
    fi
  fi
  ...
  if [[ -L "${nvim_config}" ]]; then
    ...
    if [[ "${current}" == "${lazyvim_dir}" ]]; then
      prefix_info "lazyvim already linked"
      return 0
    fi
    rm "${nvim_config}"
  elif [[ -e "${nvim_config}" ]]; then
    # Backup existing config
    local backup_dir="${HOME}/.jsh_backup/$(date +%Y%m%d_%H%M%S)"
    mkdir -p "${backup_dir}"
    mv "${nvim_config}" "${backup_dir}/"
    prefix_info "Backed up existing nvim config to ${backup_dir}/"
  fi

  mkdir -p "$(dirname "${nvim_config}")"
  ln -s "${lazyvim_dir}" "${nvim_config}"
  prefix_success "Linked lazyvim -> ~/.config/nvim"
}

# Feature flags (set during selection)
declare -g BOOTSTRAP_FEATURES_CLI=false
declare -g BOOTSTRAP_FEATURES_GUI=false
declare -g BOOTSTRAP_FEATURES_HYPRLAND=false
declare -g BOOTSTRAP_FEATURES_FONTS=false
declare -g BOOTSTRAP_FEATURES_VSCODE=false
declare -g BOOTSTRAP_FEATURES_SYSTEMD=false
declare -g BOOTSTRAP_FEATURES_NVIM=false

# Interactive feature selection for Linux
_bootstrap_select_features() {
  if ! is_linux; then
    return 0
  fi

  echo ""
  echo "${BOLD}Linux Setup Options${RST}"
  echo "${DIM}Select the features you'd like to install:${RST}"
  echo ""

  # Feature definitions: flag|label|description
  local features=(
    "cli|CLI Packages|Essential command-line tools (63 packages via dnf)"
    "gui|GUI Applications|Desktop apps via Flatpak (Brave, Discord, VS Code, etc.)"
    "hyprland|Hyprland Environment|Wayland compositor with Waybar, Wofi, Ghostty configs"
    "fonts|Nerd Fonts|JetBrains Mono Nerd Font for terminal/editor"
    "nvim|Neovim + LazyVim|Pre-built Neovim with LazyVim configuration"
    "vscode|VS Code Config|Editor settings and extensions"
    "systemd|Systemd Services|User services (gpg-agent, podman, ssh-agent)"
  )

  local selections=()
  local i=1

  echo "${CYN}Available features:${RST}"
  echo ""
  for feature in "${features[@]}"; do
    ...
    local label="${rest%%|*}"
    local desc="${rest#*|}"
    printf "  ${BOLD}%d${RST}) %-20s ${DIM}%s${RST}\n" "$i" "${label}" "${desc}"
    ((i++))
  done

  echo ""
  echo "${DIM}Options:${RST}"
  echo "  ${BOLD}a${RST}) All features (full jsh experience)"
  echo "  ${BOLD}l${RST}) Lite (dotfiles only, no packages)"
  echo "  ${BOLD}1-7${RST}) Toggle individual features (space-separated)"
  echo ""

  read -r -p "Select features [a/l/1-7]: " selection

  case "${selection,,}" in
  a | all | full)
    BOOTSTRAP_FEATURES_CLI=true
    BOOTSTRAP_FEATURES_GUI=true
    BOOTSTRAP_FEATURES_HYPRLAND=true
    BOOTSTRAP_FEATURES_FONTS=true
    BOOTSTRAP_FEATURES_NVIM=true
    BOOTSTRAP_FEATURES_VSCODE=true
    BOOTSTRAP_FEATURES_SYSTEMD=true
    ;;
  l | lite | "")
    # Lite mode - just dotfiles, all features false
    ;;
  *)
    # Parse number selections
    for num in ${selection}; do
      case "${num}" in
      1) BOOTSTRAP_FEATURES_CLI=true ;;
      2) BOOTSTRAP_FEATURES_GUI=true ;;
      3) BOOTSTRAP_FEATURES_HYPRLAND=true ;;
      4) BOOTSTRAP_FEATURES_FONTS=true ;;
      5) BOOTSTRAP_FEATURES_NVIM=true ;;
      6) BOOTSTRAP_FEATURES_VSCODE=true ;;
      7) BOOTSTRAP_FEATURES_SYSTEMD=true ;;
      esac
    done
    ;;
  esac
  ...
  echo ""
  echo "${BOLD}Selected features:${RST}"
  local any_selected=false
  [[ "${BOOTSTRAP_FEATURES_CLI}" == true ]] && {
    echo "  ${GRN}✓${RST} CLI Packages"
    any_selected=true
  }
  [[ "${BOOTSTRAP_FEATURES_GUI}" == true ]] && {
    echo "  ${GRN}✓${RST} GUI Applications"
    any_selected=true
  }
  [[ "${BOOTSTRAP_FEATURES_HYPRLAND}" == true ]] && {
    echo "  ${GRN}✓${RST} Hyprland Environment"
    any_selected=true
  }
  [[ "${BOOTSTRAP_FEATURES_FONTS}" == true ]] && {
    echo "  ${GRN}✓${RST} Nerd Fonts"
    any_selected=true
  }
  [[ "${BOOTSTRAP_FEATURES_NVIM}" == true ]] && {
    echo "  ${GRN}✓${RST} Neovim + LazyVim"
    any_selected=true
  }
  [[ "${BOOTSTRAP_FEATURES_VSCODE}" == true ]] && {
    echo "  ${GRN}✓${RST} VS Code Config"
    any_selected=true
  }
  [[ "${BOOTSTRAP_FEATURES_SYSTEMD}" == true ]] && {
    echo "  ${GRN}✓${RST} Systemd Services"
    any_selected=true
  }
  [[ "${any_selected}" == false ]] && echo "  ${DIM}(Lite mode - dotfiles only)${RST}"
  return 0
}

# Execute selected Linux features
_bootstrap_execute_features() {
  if ! is_linux; then
    return 0
  fi

  local any_features=false
  [[ "${BOOTSTRAP_FEATURES_CLI}" == true ]] && any_features=true
  [[ "${BOOTSTRAP_FEATURES_GUI}" == true ]] && any_features=true
  [[ "${BOOTSTRAP_FEATURES_HYPRLAND}" == true ]] && any_features=true
  [[ "${BOOTSTRAP_FEATURES_FONTS}" == true ]] && any_features=true
  [[ "${BOOTSTRAP_FEATURES_NVIM}" == true ]] && any_features=true
  [[ "${BOOTSTRAP_FEATURES_VSCODE}" == true ]] && any_features=true
  [[ "${BOOTSTRAP_FEATURES_SYSTEMD}" == true ]] && any_features=true

  if [[ "${any_features}" == false ]]; then
    return 0
  fi

  echo ""
  info "Installing selected features..."

  # CLI Packages
  if [[ "${BOOTSTRAP_FEATURES_CLI}" == true ]]; then
    local dnf_json="${JSH_DIR}/configs/packages/linux/dnf.json"
    if [[ -f "${dnf_json}" ]] && command -v dnf &>/dev/null; then
      local all_pkgs missing_pkgs=()
      all_pkgs=$(jq -r '.[]' "${dnf_json}" 2>/dev/null)
      ...
      if [[ ${#missing_pkgs[@]} -gt 0 ]]; then
        echo ""
        info "Installing CLI packages via dnf..."
        echo "  ${DIM}${missing_pkgs[*]}${RST}"
        echo ""
        sudo dnf install -y "${missing_pkgs[@]}" || warn "Some packages failed to install"
      fi
    fi
  fi

  # Hyprland packages (before GUI so hyprland is available)
  if [[ "${BOOTSTRAP_FEATURES_HYPRLAND}" == true ]]; then
    local hyprland_json="${JSH_DIR}/configs/packages/linux/hyprland.json"
    if [[ -f "${hyprland_json}" ]] && command -v dnf &>/dev/null; then
      local all_pkgs missing_pkgs=()
      all_pkgs=$(jq -r '.[]' "${hyprland_json}" 2>/dev/null)
      # Filter to only packages not already installed
      while IFS= read -r pkg; do
        [[ -z "${pkg}" ]] && continue
        if ! rpm -q "${pkg}" &>/dev/null; then
      ...
      if [[ ${#missing_pkgs[@]} -gt 0 ]]; then
        echo ""
        info "Installing Hyprland environment..."
        echo "  ${DIM}${missing_pkgs[*]}${RST}"
        echo ""
        sudo dnf install -y "${missing_pkgs[@]}" || warn "Some Hyprland packages failed"
      fi
    fi

    # Configure Hyprland
    if [[ -x "${JSH_DIR}/scripts/linux/configure-hyprland.sh" ]]; then
      bash "${JSH_DIR}/scripts/linux/configure-hyprland.sh"
    fi
  fi

  # GUI Applications
  if [[ "${BOOTSTRAP_FEATURES_GUI}" == true ]]; then
    echo ""
    info "Installing GUI applications (Flatpak)..."
    local flatpak_json="${JSH_DIR}/configs/packages/linux/flatpak.json"
    if [[ -f "${flatpak_json}" ]] && command -v flatpak &>/dev/null; then
      local apps
      apps=$(jq -r '.[]' "${flatpak_json}" 2>/dev/null)
      while IFS= read -r app; do
        [[ -z "${app}" ]] && continue
        if flatpak list --app --columns=application 2>/dev/null | grep -qx "${app}"; then
          echo "  ${DIM}${app} (installed)${RST}"
        else
          echo "  Installing ${app}..."
      ...
      warn "flatpak.json not found or flatpak not available"
    fi
  fi

  # Fonts (handled via Homebrew casks - font-jetbrains-mono-nerd-font)
  if [[ "${BOOTSTRAP_FEATURES_FONTS}" == true ]]; then
    echo ""
    info "Nerd Fonts will be installed via Homebrew casks"
  fi

  # Neovim + LazyVim
  if [[ "${BOOTSTRAP_FEATURES_NVIM}" == true ]]; then
    echo ""
    info "Installing Neovim from pre-built archive..."
    if [[ -f "${JSH_DIR}/src/deps.sh" ]]; then
      source "${JSH_DIR}/src/deps.sh"
      if download_binary "nvim"; then
        # Init lazyvim submodule and create symlink
        _setup_lazyvim
      else
        warn "Neovim download failed - install manually"
      fi
    else
  ...
  # VS Code
  if [[ "${BOOTSTRAP_FEATURES_VSCODE}" == true ]]; then
    echo ""
    info "Configuring VS Code..."
    if [[ -f "${JSH_DIR}/src/configure.sh" ]]; then
      source "${JSH_DIR}/src/configure.sh" 2>/dev/null
      if declare -f _configure_vscode &>/dev/null; then
        _configure_vscode false
      else
        warn "_configure_vscode function not found"
      fi
    fi
  fi

  # Systemd Services
  if [[ "${BOOTSTRAP_FEATURES_SYSTEMD}" == true ]]; then
    echo ""
    info "Configuring systemd user services..."
    if [[ -x "${JSH_DIR}/scripts/linux/configure-systemd.sh" ]]; then
      bash "${JSH_DIR}/scripts/linux/configure-systemd.sh"
    else
      warn "configure-systemd.sh not found"
    fi
  fi

  echo ""
  success "Feature installation complete!"
}

# @jsh-cmd bootstrap Clone/update repo and setup (for fresh installs)
# @jsh-opt -y,--non-interactive Skip prompts, use defaults
# @jsh-opt --full Install all features (full jsh experience)
# @jsh-opt --lite Dotfiles only (no packages)
...
cmd_bootstrap() {
  ...
  while [[ $# -gt 0 ]]; do
    ...
    -h | --help)
      ...
      echo "  --full                  Install all features (full jsh experience)"
      echo "  --lite                  Dotfiles only (no packages)"
  ...
  if [[ "${full_mode}" == true ]]; then
    ...
    BOOTSTRAP_FEATURES_CLI=false
    BOOTSTRAP_FEATURES_GUI=false
    BOOTSTRAP_FEATURES_HYPRLAND=false
    BOOTSTRAP_FEATURES_FONTS=false
    BOOTSTRAP_FEATURES_NVIM=false
    BOOTSTRAP_FEATURES_VSCODE=false
    BOOTSTRAP_FEATURES_SYSTEMD=false
  fi
  ...
  if [[ "${interactive}" == true ]]; then
    ...
    # Linux feature selection (unless --full or --lite was specified)
    if is_linux && [[ "${full_mode}" == false ]] && [[ "${lite_mode}" == false ]]; then
      _bootstrap_select_features
    fi
  else
  ...
  if [[ "${skip_git}" == true ]]; then
    ...
    if [[ -d "${JSH_DIR}" ]]; then
      ...
      local clone_hash
      clone_hash=$(git -C "${JSH_DIR}" rev-parse --short HEAD 2>/dev/null || echo "?")
      prefix_success "Cloned ${DIM}(${clone_hash})${RST}"
    fi

    # Init core submodules (lazyvim is handled separately with nvim)
    local core_submodules=(lib/fzf lib/fzf-tab lib/zsh-completions)
    if git -C "${JSH_DIR}" submodule update --init --depth 1 -- "${core_submodules[@]}" 2>/dev/null; then
      _show_submodule_status "${JSH_DIR}" "${core_submodules[@]}"
    else
