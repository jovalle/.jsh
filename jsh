...
jsh_usage() {
  ...
  printf "  %s   Discover and manage optional development tools\n" "tools      "
  printf "  %s   Sync jsh changes with remote repository\n" "sync       "
  printf "  %s   Show current environment profile and configuration\n" "profile    "
  printf "  %s   Manage shell, vim, and tmux plugins\n" "plugins    "
  echo
    ...
    case "$key" in
      ...
      --category | -c)
        ...
        if [[ -n ${2+x} ]]; then
          ...
          printf "%s\n" "--category requires an argument: --category, -c CATEGORY" >&2
          exit 1
        fi
        ;;
...
run() {
  ...
  case "$action" in
    ...
    "tools") jsh_tools_command ;;
    "sync") jsh_sync_command ;;
    "profile") jsh_profile_command ;;
    "plugins") jsh_plugins_command ;;
  esac
}
