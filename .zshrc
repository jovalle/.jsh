...
kctx+() {
  [[ -z "$1" ]] && { echo "Usage: $0 <kubeconfig>"; return 1; }
  ...
  KUBECONFIG="$KUBECONFIG_SRC" kubectl config view --minify -o jsonpath='{.contexts[*].name}' &>/dev/null || \
    { error "Invalid kubeconfig ($KUBECONFIG_SRC)"; return 1; }
  ...
  local context_name=$(KUBECONFIG="$KUBECONFIG_SRC" kubectl config view --minify -o jsonpath='{.contexts[0].name}')
  local cluster_name=$(KUBECONFIG="$KUBECONFIG_SRC" kubectl config view --minify -o jsonpath='{.contexts[0].context.cluster}')
  local user_name=$(KUBECONFIG="$KUBECONFIG_SRC" kubectl config view --minify -o jsonpath='{.contexts[0].context.user}')

  [[ -z "$context_name" ]] && { error "No context found in source kubeconfig"; return 1; }

  info "Merging context: $context_name"

  # Extract details from source
  local server=$(KUBECONFIG="$KUBECONFIG_SRC" kubectl config view --minify -o jsonpath='{.clusters[0].cluster.server}')
  local server_host=$(echo "$server" | sed -E 's|https?://([^:/]+).*|\1|')

  # Extract authinfo name from context (should be server/user format)
  local authinfo=$(KUBECONFIG="$KUBECONFIG_SRC" kubectl config view --minify -o jsonpath='{.contexts[0].context.user}')

  # Use server/user as the unique authinfo identifier
  local unique_authinfo="${server_host}/${user_name}"
  ...
  kubectl config delete-user "$unique_authinfo" &>/dev/null
  ...
  local user_cert=$(KUBECONFIG="$KUBECONFIG_SRC" kubectl config view --raw -o jsonpath='{.users[0].user.client-certificate-data}')
  local user_key=$(KUBECONFIG="$KUBECONFIG_SRC" kubectl config view --raw -o jsonpath='{.users[0].user.client-key-data}')
  ...
  if [[ -n "$user_cert" && -n "$user_key" ]]; then
    ...
    local token=$(KUBECONFIG="$KUBECONFIG_SRC" kubectl config view --raw -o jsonpath='{.users[0].user.token}')
    if [[ -n "$token" ]]; then
...
kctx-() {
  ...
  local cluster_refs=$(kubectl config view -o jsonpath="{.contexts[*].context.cluster}" | grep -c "$cluster" || true)
  local user_refs=$(kubectl config view -o jsonpath="{.contexts[*].context.user}" | grep -c "$user" || true)
