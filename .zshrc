...
kctx+() {
  ...
  KUBECONFIG_SRC="$1" KUBECONFIG_DST="$HOME/.kube/config"
  ...
  KUBECONFIG_TMP="$(dirname "$KUBECONFIG_DST")/.kubeconfig"
  info "Staging kubeconfig..."
  cat "$KUBECONFIG_DST" > "$KUBECONFIG_TMP"
  info "Merging kubeconfigs..."
  KUBECONFIG="$KUBECONFIG_SRC:$KUBECONFIG_TMP" kubectl config view --flatten > "$KUBECONFIG_DST" && \
    success "Kubeconfig merged" || { error "Failed to merge"; return 1; }
}
