...
_CLI_META_name=""               # @name
_CLI_META_version=""            # @version
_CLI_META_desc=""               # @desc
_CLI_META_usage=""              # @usage
if declare -gA _CLI_META 2>/dev/null; then
    _CLI_META=(
        [name]=""
        [version]=""
        [desc]=""
    ...
    unset _CLI_META 2>/dev/null || true
    declare -A _CLI_META=(
        [name]=""
        [version]=""
        [desc]=""
...
cli_parse() {
    ...
    while IFS= read -r line || [[ -n "${line}" ]]; do
        ...
        case "${line}" in
            @name\ *)
                _CLI_META_name="${line#@name }"
                _CLI_META[name]="${_CLI_META_name}"
                ;;
            @version\ *)
                _CLI_META_version="${line#@version }"
                _CLI_META[version]="${_CLI_META_version}"
                ;;
            @desc\ *)
                _CLI_META_desc="${line#@desc }"
                _CLI_META[desc]="${_CLI_META_desc}"
                ;;
            @usage\ *)
                _CLI_META_usage="${line#@usage }"
                _CLI_META[usage]="${_CLI_META_usage}"
                ;;
            @option\ *)
...
cli_help() {
    ...
    local name="${_CLI_META_name:-${0##*/}}"
    local version="${_CLI_META_version:-}"
    local desc="${_CLI_META_desc:-}"
    local usage="${_CLI_META_usage:-${name} [options]}"
...
_cli_print_aligned() {
    ...
    eval "local -a _arr_copy=(\"\${${_arr_name}[@]}\")"
    for item in "${_arr_copy[@]}"; do
        local name desc
...
cli_version() {
    _cli_ensure_parsed
    local name="${_CLI_META_name:-${0##*/}}"
    local version="${_CLI_META_version:-unknown}"
    printf "%s %s\n" "${name}" "${version}"
}
